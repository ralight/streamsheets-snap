...
const INTERCEPTED_MESSAGE_TYPES = [
    ...
    GatewayMessagingProtocol.MESSAGE_TYPES.GET_CELL_RAW_VALUE,
    ...
    GatewayMessagingProtocol.MESSAGE_TYPES.OPEN_MACHINE_MESSAGE_TYPE,
...
module.exports = class MachineServerInterceptor extends Interceptor {
    ...
    interceptBeforeSendToServer(message) {
        if (message && INTERCEPTED_MESSAGE_TYPES.includes(message.type)) {
            return (!message.command || !IGNORED_CMDS.includes(message.command.name));
        }
        return false;
    }
...
//# sourceMappingURL=MachineServerInterceptor.js.map